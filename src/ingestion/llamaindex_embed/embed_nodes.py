...
from utils.model_loaders import get_embedding as embed
...
def add_embeddings(chunks, dimensions=1536):
    def process_batch(batch_data, modality, max_batch_size):
        if not batch_data:
            return

        # Split large batches into smaller chunks
        for i in range(0, len(batch_data), max_batch_size):
            batch_chunk = batch_data[i : i + max_batch_size]
            _indices, content = zip(*batch_chunk)
    ...
    process_batch(text_batch, "text", 96)
    process_batch(image_batch, "image", 20)
